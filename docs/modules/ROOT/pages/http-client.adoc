...
[source,properties]
----
# Use "secureA" for all HTTP/OpenAPI tasks in the workflow "org.acme:order-flow:0.0.1"
quarkus.flow.http.client.client."org.acme:order-flow:0.0.1".name=secureA

# Override only the "fetchCustomers" task to use "internal"
quarkus.flow.http.client.client."org.acme:order-flow:0.0.1:fetchCustomers".name=internal
----

The key format is `namespace:name:version` for workflow-level overrides, or `namespace:name:version:taskName` for task-level overrides.

Resolution order for a call:
. Task-level override (`namespace:name:version:taskName`).
. Workflow-level override (`namespace:name:version`).
